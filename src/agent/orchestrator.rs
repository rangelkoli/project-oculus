...
impl Orchestrator {
    ...
    pub async fn run(&mut self, driver: &WebDriver) -> WebDriverResult<Option<String>> {
        ...
        string_response = string_response.replace("```json", "").replace("```", "").replace("\n", "");
        self.task_history.push(format!("Initial plan: {}", string_response));
        ...
        loop {
            ...
            let last_step_string = match interactive_elements {
                Ok(elements) => elements.iter().map(|(k, v)| format!("{}: {:?}", k, v)).collect::<Vec<_>>().join(", "),
            ...
            self.task_history.push(format!("Execution result: {}", result));
